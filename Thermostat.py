...
##
## This package is necessary so that we can lock the I2C bus so that we can 
## avoid collisions when multiple threads are trying to access the bus at the same time.
##
...
class TemperatureMachine(StateMachine):
    ...
    ## Thermostat with exception handling for the temperature sensor. 
    ## If the temperature sensor fails, the last known temperature will be used to determine the state of the lights.
    ##
    def updateLights(self):
        current_temp = self.getFahrenheit()
        if current_temp is None:
            current_temp = self.last_temperature_f if self.last_temperature_f is not None else 72
        ...
        if(DEBUG):
            ...
            print(f"Temp: {floor(current_temp)}")
    ...
    ##
    ## Get the temperature in Fahrenheit with exception handling for the temperature sensor. 
    ## If the temperature sensor fails, the last known temperature will be returned.
    ##
    def getFahrenheit(self):
        try:
            ...
            temp_f = ((9/5) * t) + 32
            self.last_temperature_f = temp_f
            return temp_f
        except (OSError, RuntimeError, ValueError) as exc:
            if(DEBUG):
                print(f"Temperature sensor read failed: {exc}")
            return self.last_temperature_f

    
    ##
    ##  Configure output string for the Thermostat Server with exception handling for the temperature sensor. 
    ##  If the temperature sensor fails, the last known temperature will be returned.
    ##
    def setupSerialOutput(self):
        ...
        if temp_f is None:
            temp_f = self.last_temperature_f if self.last_temperature_f is not None else 0
        output = f"{state},{floor(temp_f)},{self.setPoint}"
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                ...
                if temp_f is None:
                    temp_f = self.last_temperature_f if self.last_temperature_f is not None else 0
                lcd_line_2 = f"Temp: {floor(temp_f)}F"
    
                altCounter = altCounter + 1
            else:

                state = self.current_state.name.capitalize()
